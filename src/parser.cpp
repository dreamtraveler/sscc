...
: _phase(PARSE_PHASE_HEAD), 
  ...
  _symbols(symbols) 
{
	_lex = std::make_shared<Lex>();
}

std::shared_ptr<Token> Parser::cur() {
    ...
    while (1) {
        std::shared_ptr<Token> token = _lex->get(_input);
...
std::shared_ptr<Token> Parser::look() {
...
void Parser::parse_seg(std::shared_ptr<Token> token) {
    ...
    while (1) {
        ...
        std::shared_ptr<Token> name = token;
        ...
        std::shared_ptr<Token> value = cur();
        eat();
        if (!token_is_constant(value.get())) {
...
void Parser::prase_option() {
    ...
    std::shared_ptr<Token> token = cur();
    eat();
...
void Parser::parse_head(std::shared_ptr<Token> token) {
    switch (token->type()) {
    case TOKEN_SEGMENT:
        parse_seg(token);
        break;
...
void Parser::parse_enum() {
    ...
    while (1) {
        ...
        std::shared_ptr<Token> iden_token = token;
        loc << iden_token->loc();
        eat();
        token = cur();

		std::shared_ptr<ExprTree> expr = std::make_shared<ExprTree>();
        ...
        auto def_tree = std::make_shared<DefineTree>(iden_token, expr);
        def_tree->loc = loc;
...
void Parser::parse_body(std::shared_ptr<Token> token) {
    switch (token->type()) {
    case TOKEN_DEFINE:
        do {
			auto tree = std::make_shared<DefineTree>();
            tree->parse(this);
...
void Parser::parse_tail(std::shared_ptr<Token> token) {
}
    ...
    while (1) {
        ...
        if (*token == TOKEN_INCLUDE) {
            ...
            if (*token == TOKEN_CONST_STRING && look()->is_eol()) {
                ...
                std::shared_ptr<Path> path = std::make_shared<Path>(token->text());
                if (_input.is_root()) {
                    _symbols.exportSymbol(std::make_shared<IncludeTree>(path));
                }
                _input.load(path);
            }
            else {

...
struct Position
{
    Position() : file(nullptr), line(0), col(0) {}
    const char *file;
    unsigned line;
    unsigned col;
};
...
public:
    InputFile();
    ~InputFile();
    bool load(std::shared_ptr<Path> path);
    int cur() {
		while (_pos < _size) {
			if (_data[_pos] != '\r') {
				return _data[_pos];
...
public:
    Input();
    virtual ~Input();
    std::shared_ptr<Path> load(std::shared_ptr<Path> path, bool search = true);
...
private:
	...
	std::list<std::shared_ptr<Path>> _searchPaths;
};
